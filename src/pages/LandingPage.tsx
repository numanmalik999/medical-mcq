...
const LandingPage = () => {
  ...
  return (
    <div className="bg-background text-foreground pt-16">
          ...
          <div className="flex flex-col items-center gap-4 animate-fade-in-up delay-400">
            <div className="flex flex-col justify-center gap-4 w-full max-w-md lg:max-w-none lg:flex-row">
              <Link to={user ? "/user/dashboard" : "/subscription"} className="w-full lg:w-auto">
                <Button size="lg" className="bg-primary-foreground text-primary hover:bg-primary-foreground/90 w-full min-w-[200px] text-base font-bold shadow-lg">
                  {user ? "Go to Dashboard" : settings.hero.ctaPrimaryText}
                </Button>
              </Link>
              <Link to="/quiz" className="w-full lg:w-auto">
                <Button size="lg" variant="secondary" className="flex items-center gap-2 w-full min-w-[200px] text-base font-bold shadow-md">
                  <BookOpenText className="h-5 w-5" /> {settings.hero.ctaSecondaryText}
                </Button>
              </Link>
              <Link to="/quiz-of-the-day" className="w-full lg:w-auto">
                <Button size="lg" className="flex items-center gap-2 w-full min-w-[200px] text-base font-bold bg-white text-black hover:bg-white/90 shadow-md border-2 border-white">
      ...
      <section className="py-6 bg-muted/50 border-y" aria-label="Key Statistics">
      ...
      {/* Features Section */}
      <section className="py-12 md:py-16 bg-background" aria-label="Platform Features">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl md:text-4xl font-bold mb-3">Master Your Licensing Journey</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {settings.features.map((feature, index) => (
              <Card key={index} className="flex flex-col items-center p-6 text-center hover:shadow-lg transition-all border-none shadow-sm bg-muted/30">
      ...
      <section className="py-8 bg-white border-y" aria-label="Official Licensing Resources">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-lg font-bold mb-4 text-muted-foreground uppercase tracking-widest">Official Licensing Resources</h2>
          <div className="flex flex-wrap justify-center gap-4 md:gap-8">
            {[
              ...
              <a 
                ...
                className="flex items-center gap-2 text-xs font-semibold text-slate-500 hover:text-primary transition-colors group"
              >
                {body.name}
      ...
      {recentBlogs.length > 0 && (
        <section className="py-12 md:py-16 bg-muted/20" aria-label="Latest Blog Posts">
          <div className="container mx-auto px-4">
            <div className="flex justify-between items-end mb-8">
              <div className="text-left">
                <h2 className="text-2xl md:text-3xl font-bold mb-1">Expert Exam Guides</h2>
                <p className="text-sm text-muted-foreground">In-depth insights into the Gulf licensing process.</p>
              </div>
              ...
              {recentBlogs.map((blog) => (
                <Card key={blog.slug} className="flex flex-col h-full hover:shadow-md transition-shadow overflow-hidden">
                  ...
                  <CardHeader className="flex-grow p-4">
                    ...
                    <CardDescription className="text-xs line-clamp-2">{blog.meta_description}</CardDescription>
                  </CardHeader>
                  <CardFooter className="p-4 pt-0">
                    <Button asChild variant="link" size="sm" className="p-0 h-auto">
                      <Link to={`/blog/${blog.slug}`} className="flex items-center text-[11px] font-bold uppercase tracking-tighter">Read More <ArrowRight className="ml-1.5 h-3 w-3" /></Link>
                    </Button>
                  </CardFooter>
                </Card>
              ))}
      ...
      {!hideSubscriptionSection && (
        <section className="py-12 md:py-16 bg-slate-900 text-white" aria-label="Pricing and Subscription Plans">
          <div className="container mx-auto px-4 text-center">
            <h2 className="text-3xl md:text-4xl font-bold mb-3 text-white">{settings.pricingCta.title}</h2>
            <p className="text-base text-slate-300 mb-10 max-w-2xl mx-auto">
              ...
              {subscriptionTiers.map((tier) => (
                <Card key={tier.id} className="flex flex-col text-left shadow-2xl transition-all border-slate-800 bg-slate-950 scale-100 hover:scale-[1.02]">
                  <CardHeader className="pb-4 p-5">
                    ...
                    <CardDescription className="text-slate-400 text-xs line-clamp-2 h-8">{tier.description}</CardDescription>
                  </CardHeader>
                  <CardContent className="flex-grow space-y-4 border-b border-slate-800 pb-6 p-5">
                    <div className="flex items-baseline">
                      <span className="text-2xl font-extrabold text-white">{tier.currency} {tier.price.toFixed(2)}</span>
                      <span className="text-[10px] text-slate-400 ml-1.5 uppercase font-bold">/ {tier.duration_in_months} mo</span>
                    </div>
                    {tier.features && tier.features.length > 0 && (
                      <ul className="space-y-2">
                        {tier.features.slice(0, 5).map((feature, index) => (
                          <li key={index} className="flex items-start gap-2 text-slate-300 text-[10px] font-medium leading-tight">
                  ...
                  <CardFooter className="pt-4 p-5">
                    <Link to={user ? `/user/payment/${tier.id}?priceId=${tier.stripe_price_id}` : `/signup?tierId=${tier.id}`} className="w-full">
                      <Button className="w-full h-9 text-xs bg-white text-slate-900 hover:bg-slate-200 font-bold uppercase tracking-wider" disabled={!tier.stripe_price_id && !!user}>
      ...
      <section className="py-12 md:py-16 bg-primary text-primary-foreground text-center" aria-label="Newsletter Signup">
        <div className="container mx-auto px-4 max-w-3xl">
          ...
          <Form {...marketingForm}>
            <form onSubmit={marketingForm.handleSubmit(handleMarketingSubscribe)} className="flex flex-col sm:flex-row gap-3 max-w-lg mx-auto">
              <FormField
                  ...
                  <FormItem className="flex-grow">
                    <FormControl>
                      <Input
                        ...
                        className="h-12 text-base bg-white text-black border-none"
                        disabled={isSubscribing}
                        {...field}
                      />
                    </FormControl>
                    <FormMessage className="text-left text-white font-bold text-xs" />
                  </FormItem>
                )}
              />
              <Button type="submit" size="lg" variant="secondary" className="h-12 text-sm px-6 w-full sm:w-auto font-bold uppercase tracking-wider" disabled={isSubscribing}>
